...
# In[6]:
def solveMin_fitCosnt(subject, i, j, k, bsizes, loader_func, G, intercept=None, scale=2, max_iters=1500, verbose=False, prob=None, saveImg=False):
    measures = ['mse', 'mse1000', 'mse2000', 'mse3000']
    info = dict((key, []) for key in measures)
    seg = 0
    for b_index in xrange(len(bsizes)):
        ...
        if saveImg:
            print '$$ saving recontructed image of group', group_number_job, 'in', base_folder + 'A_g%d_bsize%f' % (group_number_job, b_index)
            np.save(base_folder + 'A_g%d_bsize%d' % (group_number_job, b_index), A)
...
def try_bsize(subject, i, j, k, bsize, loader_func, G, intercept=None, scale=2, max_iters=1500, verbose=False):
    definition_fun = None
...
def try_value(i_hr,M, Nx, Ny, Nz, Nb, Nc, b1000_index, b2000_index, b3000_index, definition_fun, max_iters, verbose, i=-1, res=None):
    ...
    print 'Solving problem bsize:', i_hr.shape[3]
    for i in xrange(2):
        prob.solve(solver='SCS', max_iters=5, eps=1.0e-05, verbose=verbose)  # Returns the optimal value.
...
RES_BASE_FOLDER = '/home/lgomez/workspace/iqt/results/exp8/'
...
for i, j, k in it:
    for group_num in [group_number_job]:
        ...
        for subject_index in [fit_index_job]:
            ...
            saveImg = group_number_job == fit_index_job
            info, seg =\
            ...
            mse[:] = info['mse']
            mse1000[:] = info['mse1000']
            mse2000[:] = info['mse2000']
            mse3000[:] = info['mse3000']
...
min_bsize = bsizes[index]
print 'rangos:', bsizes
print 'MIN bsizes=', min_bsize
