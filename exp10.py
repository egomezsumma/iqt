...
# In[6]:
def solveMin_fitCosnt(subject,i,j,k, loader_func, G, intercept=None, scale=2, verbose=False, prob=None):
    ...
    print '$$ saving original image of group', group_number_job, 'in', base_folder + 'i_hr_g%d__%d_%d_%d' % (group_number_job, i,j,k)
    np.save(base_folder + 'i_hr_g%d__%d_%d_%d' % (group_number_job,i,j,k), i_hr)
...
def try_value(i,j,k, i_hr, i_lr,M, Nx, Ny, Nz, Nb, Nc, b1000_index, b2000_index, b3000_index, definition_fun, max_iters=1000, verbose=False, res=None):
    ...
    for key in parameters.keys():
        print t3, key, '=',  parameters[key].value, '    ', datetime.datetime.now()
    sys.stdout.flush()
    ...
    print '$$ saving recontructed image of group', group_number_job, 'in', base_folder + 'A_g%d__%d_%d_%d' % (group_number_job, i,j,k)
    np.save(base_folder + 'A_g%d__%d_%d_%d' % (group_number_job, i,j,k), A)
...
RES_BASE_FOLDER = '/home/lgomez/workspace/iqt/results/exp10/'
...
base_folder = RES_BASE_FOLDER + formula_to_use + '/'
...
it = d.DmriPatchIterator(range(fila_job, 12, 12), range(7, 14, 14), range(0, 12, 1))
for i, j, k in it:
    print 'Doin patch:', i, j, k
    for group_num in [group_number_job]:
        ...
        for subject_index in [fit_index_job]:
            ...
            print t1, '= Solving optimization problem (subject: %s, fila: %d) === ' % (subject, fila_job), datetime.datetime.now()
            sys.stdout.flush()
